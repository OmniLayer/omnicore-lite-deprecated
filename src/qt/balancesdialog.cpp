    ...
    QDialog(parent), ui(new Ui::balancesDialog), clientModel(0), walletModel(0)
{
    // setup
    ui->setupUi(this);
    ui->balancesTable->setColumnCount(3);
    ...
    connect(balancesCopyAvailableAmountAction, SIGNAL(triggered()), this, SLOT(balancesCopyCol2()));
}
...
void BalancesDialog::AddRow(const std::string& label, const std::string& address, const std::string& available)
{
    ...
    ui->balancesTable->setItem(workingRow, 2, availableCell);
}
...
void BalancesDialog::PopulateBalances(unsigned int propertyId)
{
    ...
    if(propertyId==2147483646) {
        ...
        for(std::unordered_map<string, CMPTally>::iterator my_it = mp_tally_map.begin(); my_it != mp_tally_map.end(); ++my_it) {
            ...
            // add the row
            if (!watchAddress) {
                AddRow(GetAddressLabel(my_it->first), address, availableStr);
            } else {
                AddRow(GetAddressLabel(my_it->first), address + " (watch-only)", availableStr);
            }
        }
    }
}
